...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      transport: "wechat-adapter",
    });
  } catch (error) {
    const message =
      error instanceof Error ? error.message : "Couldn't load the public WeChat bot summary.";
...
export async function POST(request: Request) {
  try {
    ...
    const message =
      error instanceof Error ? error.message : "Couldn't run the public WeChat bot command.";
