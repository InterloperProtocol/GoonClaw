...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      transport: "private-telegram-adapter",
    });
  } catch (error) {
    const message =
      error instanceof Error ? error.message : "Couldn't load the private bot summary.";
...
export async function POST(request: Request) {
  try {
      ...
      const runtime = setTianshiRuntimeControl({
        ...
        note:
          ...
          (payload.command === "enable_tianshi"
            ? "Enabled from the private operator bot."
            : "Paused from the private operator bot."),
    ...
    const message =
      error instanceof Error ? error.message : "Couldn't run the private bot command.";
    const status =
