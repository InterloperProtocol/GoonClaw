...
export function getVendoredGoonclawSkillNames(skillRoot: string) {
...
export function getConfiguredRepoMcpServerNames() {
  ...
  const mcpDir = path.resolve(process.cwd(), "services/goonclaw-automaton/mcp");
    ...
    readdirSync(mcpDir, { withFileTypes: true })
      .filter(
        (entry) =>
          ...
          entry.name !== "goonclaw-codex.config.json",
      )
